...
export default function CandidateCard({candidate, allCandidatures = [], onStatusChange}: CandidateCardProps) {
    ...
    const applicantsForThisMission = allCandidatures
        .filter(c => {
            ...
            return currentCardOffreId && targetOffreId && currentCardOffreId === targetOffreId;
        })
        .map(c => {
    ...
    const handleValid = (idCandidature: number) => {
        CandidateService.valider(String(idCandidature)).then(() => {
    ...
    const handleDenied = (idCandidature: number) => {
        CandidateService.refuser(String(idCandidature)).then(() => {
            onStatusChange(idCandidature, "REFUSE" as Statut);
        }).catch(err => console.error("Erreur refus:", err));
    };

    const isAlreadyAssigned = applicantsForThisMission.some(app => app.statut_candidature === "VALIDE");

    return (
        <div
            className="flex flex-col gap-4 p-5 bg-base-200 border border-gray-100 dark:border-neutral-800 rounded-xl shadow-sm transition-all hover:shadow-md">

            <div className="flex items-center justify-between w-full text-[10px]">
                <span className={`px-2.5 py-1 rounded-md font-black uppercase tracking-wider text-[9px] ${
                ...
                <span className="font-medium text-gray-400 dark:text-gray-500">
                    Posté le {candidate?.mission?.date ? new Date(candidate?.mission?.date).toLocaleDateString("fr-FR", {
                        day: "numeric",
                        month: "short",
                        year: "numeric"
                    })
            ...
            <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2 text-xs font-semibold text-gray-500 dark:text-gray-400">
                    <span className="font-normal text-gray-400">Par {candidate?.client?.prenom}</span>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full">
                ...
                <dialog ref={modalRef} className="modal modal-bottom sm:modal-middle">
                    <div
                        className="modal-box bg-base-100 dark:bg-neutral-900 text-left rounded-t-2xl sm:rounded-2xl max-w-2xl h-[75vh] flex flex-col p-6">
                        ...
                        <div className="mb-4 pr-8">
                            <h3 className="font-black text-xl text-gray-900 dark:text-white">Candidatures reçues</h3>
                            <p className="text-xs text-gray-400 mt-0.5">Pour l'annonce : {candidate?.mission?.titre}</p>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col gap-3 overflow-y-auto no-scrollbar py-2 pr-1">
                            {applicantsForThisMission.length > 0 ? (
                                applicantsForThisMission.map((applicant) => (
                                    <div key={applicant.id_candidature}
                                        ...
                                        <div className="flex items-center justify-between w-full">
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 font-medium line-clamp-1">{applicant.metier}</p>
                                            </div>
                                            ...
                                            <span
                                                ...
                                                {applicant.statut_candidature === 'VALIDE' ? 'Choisi' : applicant.statut_candidature === 'REFUSE' ? 'Refusé' : 'En attente'}
                                            </span>
                                        </div>

                                        <div
                                            className="grid grid-cols-3 gap-2 w-full pt-2 border-t border-gray-100 dark:border-neutral-800/60">
                                            <button
                                                type="button"
                                                onClick={() => navigateToUserProfile(applicant.id_utilisateur)}
                                            ...
                                            {applicant.statut_candidature === "EN_ATTENTE" && !isAlreadyAssigned ? (
                                                <>
                                                    <button
                                                        type="button"
                                                        onClick={() => handleDenied(applicant.id_candidature)}
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => handleValid(applicant.id_candidature)}
                                                ...
                                                <div
                                                    className="col-span-2 flex items-center justify-center text-[10px] text-gray-400 italic font-medium bg-gray-50 dark:bg-neutral-800/20 rounded-lg">
                                                    {applicant.statut_candidature === "VALIDE"
                                                        ? "Candidat retenu 🎉"
                                                        : isAlreadyAssigned
                                                            ? "Offre pourvue"
                                ...
                                <div
                                    className="flex-1 flex flex-col items-center justify-center text-center py-6 text-xs text-gray-400">
