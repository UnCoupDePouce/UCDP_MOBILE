...
export default function DetailMissionPage() {
    ...
    const handleApply = () => {
        const idClientProprietaire = mission?.utilisateur?.id_utilisateur || mission?.id_utilisateur;
    ...
    return (
        <>
            <HeaderMissionDetail
                name={`${mission?.utilisateur?.prenom || ""} ${mission?.utilisateur?.nom || ""}`.trim() || "Détail"}
                loading={loading}/>
            {loading ? (
                ...
                <>
                    <div
                        className="relative w-full h-[40vh] shrink-0 bg-indigo-600 flex items-end justify-start overflow-hidden">
                <span
                    className="text-[50vw] font-black text-black/10 leading-[0.75] select-none tracking-tighter ml-[-2vw] mb-[-4vw]">
                    {`${mission?.utilisateur?.nom?.split(' ')[0]?.[0] || ''}${mission?.utilisateur?.prenom?.split(' ')[0]?.[0] || ''}`.toUpperCase()}                </span>
                    </div>
            ...
            {mission?.localisation || "France"}, France
                                ...
                                >
                                    ...
                                    <span
                                        className="text-[10px] font-black uppercase tracking-wide transition-colors underline group-hover:text-orange-500">
            {mission?.utilisateur?.prenom} {mission?.utilisateur?.nom}
                            ...
                            <div>
                                {mission?.image && mission.image.length > 0 ? (
                                    <div className="grid grid-cols-3 gap-2">
                                        <div
                                            className="bg-black aspect-square flex flex-col items-center justify-center text-center p-2 rounded-sm">
                                        <span className="text-orange-500 font-black text-2xl leading-none">
                                            {String(mission.image.length).padStart(2, '0')}
                                        </span>
                                            <span
                                                className="text-white text-[7px] font-black uppercase tracking-widest mt-1">
                                            Photos
                                        </span>
                                        </div>
                        ...
                        {((role === "PRESTATAIRE" || role === "ADMIN") && !mission?.statut && !alreadyApplying) && (
                            <div
                                ...
                                <button disabled className="btn btn-soft btn-info whitespace-nowrap">
                                    Est. : {mission?.prix} €
                                </button>
                                <button
                                    onClick={handleApply}
