      ...
      <div className="min-h-screen bg-white dark:bg-neutral-950 transition-colors duration-300">
        ...
        <main className="px-6 py-4 pb-32">
          <div className="mb-6 flex justify-end">
            <button onClick={toggleTheme} className="relative w-14 h-8 bg-gray-100 dark:bg-neutral-800 rounded-full p-1 border border-gray-200 dark:border-neutral-700">
              <div className={`absolute top-1 left-1 size-6 rounded-full bg-white dark:bg-neutral-950 shadow-sm flex items-center justify-center transition-transform duration-300 ${isDark ? "translate-x-6" : "translate-x-0"}`}>
                <IonIcon name={isDark ? "moon" : "sunny"} className={isDark ? "text-indigo-400" : "text-amber-500"} style={{ fontSize: "14px" }} />
              </div>
            </button>
          </div>
          ...
          <div className="flex flex-col gap-y-3">
            ...
            <InfoRow icon="location-outline" label="Localisation" value={`${user?.ville}, ${user?.code_postal}`} />
            {isProMode && (
