...
export default function Candidate() {
    ...
    const cartesMissionsUniques = candidates.filter(
        (candidature, index, tableId) =>
            tableId.findIndex(c => c.mission.id_offre === candidature.mission.id_offre) === index
    );

    const counts = {
        tous: cartesMissionsUniques.length,
        accepte: cartesMissionsUniques.filter(c => c.statut === "VALIDE").length,
        enCours: cartesMissionsUniques.filter(c => c.statut === "EN_ATTENTE").length,
    ...
    const updateCandidateStatus = (idCandidature: number, newStatut: Statut) => {
        setCandidates(prev =>
            prev.map(c => c.id_candidature === idCandidature ? {...c, statut: newStatut} : c)
        );
    };
    ...
    return (
        <>
            ...
            <Layout>
                <div
                    className="flex flex-row items-center gap-2 w-full mb-6 overflow-x-auto no-scrollbar scroll-smooth px-2">
                    <button type="button" onClick={() => setActiveTab("tous")} className={getButtonClass("tous")}>
                        <span>Tous</span>
                        <span className={getBadgeClass("tous")}>{counts.tous}</span>
                    </button>

                    <button type="button" onClick={() => setActiveTab("accepte")} className={getButtonClass("accepte")}>
                        <span>Accepté</span>
                        <span className={getBadgeClass("accepte")}>{counts.accepte}</span>
                    </button>

                    <button type="button" onClick={() => setActiveTab("enCours")} className={getButtonClass("enCours")}>
                        <span>En cours d'acceptation</span>
                        <span className={getBadgeClass("enCours")}>{counts.enCours}</span>
                    </button>
                    {role != "CLIENT" && (
                        <button type="button" onClick={() => setActiveTab("refuse")}
